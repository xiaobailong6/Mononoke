...
const main = (config)  =>{
  ...
  // 覆盖通用配置
  config["mixed-port"] = 7890;  // 数字类型
  config["tcp-concurrent"] = true;
  config["allow-lan"] = true;
  config["ipv6"] = true;
  config["log-level"] = "info";
  config["unified-delay"] = true; // 布尔值
  config["find-process-mode"] = "strict";
  config["global-client-fingerprint"] = "chrome";

  // 覆盖 dns 配置（国内走阿里，国外走谷歌）
  config["dns"] = {
    "enable": true,
    "listen": "0.0.0.0:1053",
    "ipv6": true,
  ...
  config["proxy-groups"] = [
    ...
    },
    {
      ...
      "name": "Proxy",
      "type": "select",
      "proxies": ["HongKong", "TaiWan", "Japan", "Singapore", "America", "United Kingdom", "Korea","Germany", "Russia", "AllServer", "DIRECT"],
